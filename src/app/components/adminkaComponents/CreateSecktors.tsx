...
export default function CreateSectors() {
    ...
    const getSectors = async () => {
        try {
            const response = await axios.get('/api/adminka/updateSector');
    ...
    const handleAddSector = async () => {
        ...
        try {
            await axios.post('/api/adminka/updateSector', { name: sectorName });
    ...
    const handleEditSector = async () => {
        ...
        try {
            await axios.put('/api/adminka/updateSector', { sectorId: selectedSector.id, name: sectorName });
    ...
    const handleDeleteSector = async (clickId) => {
        ...
        try {
            await axios.delete('/api/adminka/updateSector', { data: { sectorId: clickId } });
    ...
    return (
        <Box sx={{ height: '97%', p: 4, backgroundColor: '#fff', borderRadius: 2, boxShadow: 2 }}>
            ...
            <Dialog open={openDialog} onClose={handleDialogClose}>
                <DialogTitle>{dialogAction === 'add' ? 'Добавить участок' : 'Редактировать участок'}</DialogTitle>
                <DialogContent>
                    <TextField
