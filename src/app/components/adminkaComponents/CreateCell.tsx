...
export default function CreateCell() {
    ...
    const getSectors = async () => {
        try {
            const response = await axios.get('/api/adminka/updateCell');
    ...
    const handleAddSector = async () => {
        ...
        try {
            await axios.post('/api/adminka/updateCell', { name: sectorName });
            showSnackbar('Ячейка успешно добавлена!', 'success');
            getSectors();
            handleDialogClose();
        } catch {
            showSnackbar('Ошибка при добавлении Ячейки.', 'error');
        }
    };
    ...
    const handleEditSector = async () => {
        ...
        try {
            await axios.put('/api/adminka/updateCell', { sectorId: selectedSector.id, name: sectorName });
            showSnackbar('Ячейка успешно обновлёна!', 'success');
            getSectors();
            handleDialogClose();
        } catch {
...
// @ts-ignore
    const handleDeleteSector = async (clickId) => {
        ...
        try {
            await axios.delete('/api/adminka/updateCell', { data: { sectorId: clickId } });
            showSnackbar('Ячейка удалёна!', 'success');
            getSectors();
            
        } catch {
            showSnackbar('Ошибка при удалении Ячейки.', 'error');
        }
    };
    ...
    return (
        <Box sx={{ height: '97%', p: 4, backgroundColor: '#fff', borderRadius: 2, boxShadow: 2 }}>
            ...
            <Dialog open={openDialog} onClose={handleDialogClose}>
                <DialogTitle>{dialogAction === 'add' ? 'Добавить ячейку' : 'Редактировать ячейку'}</DialogTitle>
                <DialogContent>
                    <TextField
