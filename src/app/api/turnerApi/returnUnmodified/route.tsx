...
export async function POST(req: NextRequest) {
  try {
    ...
    const turnerId = Number(data.get('turnerId')); // Преобразуем userId в число
    const turnerInstrumentId = Number(data.get('turnerInstrumentId')); // Преобразуем userId в число
    const existingInstrumentId = Number(data.get('existingInstrumentId')); // Преобразуем userId в число
    ...
    const storageCells = JSON.parse(data.get('storageCells') as string); // Массив объектов { storageCellId: number, quantity: number }
    // const machineIds = JSON.parse(data.get('machineIds') as string) as number[];
    // const file = data.get('file') as File | null;

    // Проверяем, что storageCellsData содержит числа
    if (!Array.isArray(storageCells) || !storageCells.every(cell =>
      typeof cell.id === 'number' && typeof cell.quantity === 'number'
    )) {
    ...
    const quantity = storageCells.reduce((sum, cell) => sum + cell.quantity, 0);
    ...
    const instrumentData = {
      ...
      status: null,
      quantity,
      storageCells, // Массив ячеек хранения и количеств
    ...
    const newInstrument = await prisma.instrumentTransactionTurner(instrumentData);
