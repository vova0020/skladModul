...
export async function GET() {
    try {
        const newOrder = await prisma.getAdminSectors();
...
export async function PUT(req: NextRequest) {
    try {
        ...
        const updatedSectors = await prisma.putAdminSectors(sectorId, name);
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        // Обновляем запись в базе данных с помощью Prisma
        const updatedSectors = await prisma.delAdminSectors(sectorId);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const newOrder = await prisma.createAdminSectors(data);
        // console.log(data);
