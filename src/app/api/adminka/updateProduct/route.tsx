...
export async function GET() {
    try {
        const newOrder = await prisma.getAdminProducts();
...
export async function PUT(req: NextRequest) {
    try {
        ...
        const updatedSectors = await prisma.putAdminProducts(sectorId, name);
...
export async function DELETE(req: NextRequest) {
    try {
        ...
        // Обновляем запись в базе данных с помощью Prisma
        const updatedSectors = await prisma.delAdminProducts(sectorId);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const newOrder = await prisma.createAdminProducts(data);
        // console.log(data);
