...
export default class prismaInteraction {
    ...
    // Изменение станка
    async putAdminStanock(stanockId: number, name: string, productId: number[], sectionId: number) {
        try {
    ...
    async createAdminStanock(data: any) {
        ...
        try {
            ...
            const newMachine = await prisma.machine.create({
                data: {
                    name: data.name,
                    section: { connect: { id: data.sectionId } }, // Связь с участком
                    product: {
    ...
    async createAdminInstrument(data: any) {
        try {
            ...
            if (!data.name || !data.type || !data.storageCellsData || !data.machineIds) {
    ...
    async createInstrumentTransaction(data) {
        ...
        try {
            // Проверка для операции возврата
            if (type === 'return') {
                const summary = await prisma.instrumentSummary.findUnique({
                    where: {
                        instrumentId_sectionId_machineId: {
                            instrumentId,
                            sectionId: sectionId || null,
                ...
                const availableForReturn = totalIssued - totalReturned;

                if (quantity > availableForReturn) {
                    throw new Error(`Невозможно вернуть ${quantity} инструментов. Максимум можно вернуть: ${availableForReturn}`);
                }
            }
            ...
            await this.updateSummary(instrumentId, sectionId, machineId, type, quantity);
            ...
            for (const cell of storageCells) {
                ...
                if (toolCell) {
                    const newQuantity = type === 'issue' ? toolCell.quantity - cellQuantity : toolCell.quantity + cellQuantity;
    ...
    // Функция для расчета сколько списали с сколько сдали в ремонт
    async updateSummaryWriteOffRepair(
        instrumentId: number,
        type: 'write_off' | 'repair' | 'repairOff', // Добавлен новый тип 'repairOff'
        quantity: number
    ) {
        ...
        if (existingSummary) {
            ...
            summary = await prisma.instrumentSummaryWriteOffRepair.update({
                ...
                data: {
                    ...
                    totalRemont: type === 'repair' ? { increment: quantity } :
                        type === 'repairOff' ? { decrement: quantity } : undefined,
                },
            });
        } else {
            ...
            if (type === 'repairOff') {
                throw new Error('Cannot decrement totalRemont for a non-existing record');
            }
            ...
            summary = await prisma.instrumentSummaryWriteOffRepair.create({
                data: {
                    ...
                    totalRemont: type === 'repair' ? quantity : 0,
                },
            });
        }
...
async createInstrumentTransactionRemontOF(data) {
    const {
        ...
        userId,
        transactionType = data.operationType,
        reason = 'Инструмент после ремонта',
        cells,
    } = data;
